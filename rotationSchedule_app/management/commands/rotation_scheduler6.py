...
model.u = Var(model.Couples,model.W) # Couple matching variables
model.v = Var(model.Couples,model.W) # Couple matching variables



####Fix the value of these variables
#####Zabs for last week in each block is zero
#def ZabsFix_rule(model,d,r,w):
#	if b in model.DBlocks[d]:
#		if w not in model.BWeeksMinus1[b]:
#			return model.Zabs[d,r,w] == 0
#	return Constraint.Skip
#model.ZabsFix2Zero = Constraint(model.D, model.R, model.B, model.W, rule = ZabsFix_rule)
#
######X for blocks not corresponding to the residents year are zero
#def XFix_rule(model,d,r,b):
#	if b not in model.DBlocks[d]:
#		return model.X[d,r,b] == 0
#	return Constraint.Skip
#model.XFix2Zero = Constraint(model.D, model.R, model.B, rule = XFix_rule)

#######Objective##############
def Objective_rule(model):
	return sum(sum(sum(model.Z[d,r,w]*model.P[d,r,w] for w in model.W) for r in model.R) + model.Zhat[d] for d in model.D) - sum(sum(u[i,j,w]+v[i,j,w] for w in model.W) for (i,j) in model.Couples)
model.Total_Utility = Objective(rule=Objective_rule, sense=maximize)

#######Constraints#########
### Todo: Couple matching constraints
#Clinic Constraints
