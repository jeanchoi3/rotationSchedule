...
model.windowSize=Param(within=NonNegativeIntegers,default=6)
...
model.minClinicWeeks = Param(within=NonNegativeIntegers, default=2) #min number of clinic weeks in each window
...
#model.DTracks = Param(model.D,within=model.T,default = 'noTrack') ## Track of each doctor 
### Vacation Parameters
...
model.Zhat = Var(model.D,within=NonNegativeIntegers,bounds=(model.minClinicWeeks,None)) #Variable needed in order to maximize number of clinic weeks within each window
...
#def min_track(model, d, r):
#	if model.DTracks[d]!='noTrack':
#		return sum(model.Z[d,r,w] for w in model.W) >= model.MinTrack[model.DTracks[d], r]
#	return Constraint.Skip
#model.minTrackConstraint = Constraint(model.D, model.R, rule=min_track)

#def max_track(model, d, r):
#	if model.DTracks[d]!='noTrack':
#		return sum(model.Z[d,r,w] for w in model.W) <= model.MaxTrack[model.DTracks[d], r]
#	return Constraint.Skip
#model.maxTrackConstraint = Constraint(model.D, model.R, rule=max_track)
